...
if [ -z "${PJS}" ]; then
    log "Fill the value of 'PJS' of db.sh"
    exit 1
fi
...
if [ ${stage} -le 0 ] && [ ${stop_stage} -ge 0 ]; then
    ...
    # The PJS data should be downloaded from https://sites.google.com/site/shinnosuketakamichi/research-topics/pjs_corpus
    # with authentication
...
if [ ${stage} -le 1 ] && [ ${stop_stage} -ge 1 ]; then
    ...
    python local/dataset_split.py ${PJS} \
